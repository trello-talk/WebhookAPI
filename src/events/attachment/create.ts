  ...
  async onEvent(data) {
    ...
    return data.send({
      default: {
        ...
        image: data.attachment.url.startsWith(IMAGE_ATTACHMENT_HOST) ? { url: data.attachment.url } : null
      },
      small: {
        ...
        thumbnail: data.attachment.url.startsWith(IMAGE_ATTACHMENT_HOST) ? { url: data.attachment.url } : null
      }
    });
  }
};
